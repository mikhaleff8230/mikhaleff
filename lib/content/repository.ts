...
export async function getHomepage(locale: string): Promise<HomepageContent> {
  ...
  try {
    ...
    return {
      ...
      film: { ...fallbackHomepage.film, enabled: entry.filmEnabled !== false, src: entry.filmSrc || undefined, poster: { ...image(entry.filmPosterSrc, entry.filmPosterAlt, fallbackHomepage.film.poster), width: entry.filmPosterWidth, height: entry.filmPosterHeight }, eyebrow: entry.filmEyebrow || fallbackHomepage.film.eyebrow, title: entry.filmTitle || fallbackHomepage.film.title, caption: entry.filmCaption || fallbackHomepage.film.caption, linkLabel: entry.filmLinkLabel || fallbackHomepage.film.linkLabel },
