...
export const exhibition = defineType({
  ...
  preview: { select: { title: "title.0.value", date: "startDate", city: "city", media: "cover" }, prepare: ({ title, date, city, media }) => ({ title, subtitle: [date?.slice(0, 4), city].filter(Boolean).join(" · "), media }) },
});
